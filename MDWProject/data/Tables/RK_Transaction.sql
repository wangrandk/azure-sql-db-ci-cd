CREATE TABLE [data].[RK_Transaction] (
...
CREATE NONCLUSTERED INDEX [IX_RK_Transaction_Reporting_MsgReportDate]
    ON [data].[RK_Transaction]([MsgReportDate] ASC);


GO

CREATE NONCLUSTERED INDEX [IX_RK_Transaction_Reporting]
    ON [data].[RK_Transaction]([ContractorID] ASC, [EquipmentType] ASC, [TransactionBusinessDate] ASC)
    INCLUDE([MessageReceptionDate], [DeviceID], [ServiceProviderID], [VehicleID]);


GO

CREATE NONCLUSTERED INDEX [IX_RK_Transaction_Reporting_MsgRTransTFareI]
    ON [data].[RK_Transaction]([MsgReportDate] ASC, [FareIndicator] ASC, [TransactionType] ASC);


GO

CREATE COLUMNSTORE INDEX [CSIX_RK_Transaction]
    ON [data].[RK_Transaction]([TransactionID], [MsgReportDate], [EquipmentType], [BusinessDay], [JourneyRef], [JourneyPointRef], [JourneySequenceNumber], [PassengerGroupType2], [PassengerGroupTotal2], [PassengerGroupType3], [PassengerGroupTotal3], [OperationAction], [OperationType], [TripID], [TripTimeDeparture], [PaymentMeansType], [ValidationModel], [PassengerGroupType1], [PassengerGroupTotal1], [MediaType], [CardEngravedID], [ShiftNumber], [StaffID], [VehicleID], [RouteID], [TransactionValue], [TransactionMediaLastDeviceID], [ServiceProviderID], [PreviousPTO], [ContractorID], [StopPointID], [TransactionStatus], [DeviceID], [MsgSequenceNumber], [ApplicationTransactionSequenceNumber], [TransactionType], [TransactionBusinessDate]);
